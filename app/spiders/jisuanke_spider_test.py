from app.spiders.jisuanke_spider import JisuankeSpider
from app.test_base import client


def test_jisuanke_user_info(client):
    from app import create_app
    from app.models.oj_username import OJUsername
    create_app().app_context().push()
    oj_username = OJUsername()
    oj_username.oj_username = '4lkvgc2'

    test_data = {
        'oj': 'jisuanke',
        'problem_pid': 'T1001',
        'accept_time': '2019-12-06 10:58:07'
    }

    accept_problems = {}
    res = JisuankeSpider().get_user_info(oj_username, accept_problems)
    assert test_data in res['data']

    accept_problems = {'jisuanke-T1001': '2019-12-06 10:58:07'}
    res = JisuankeSpider().get_user_info(oj_username, accept_problems)
    assert test_data not in res['data']
